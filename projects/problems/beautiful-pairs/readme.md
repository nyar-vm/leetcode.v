...
- **LeetCode**：[#2613 Beautiful Pairs](https://leetcode.com/problems/beautiful-pairs/)
- **难度**：Hard
- **标签**：Geometry · Array · Math · Divide and Conquer · Ordered Set · Sorting
...
2 $\le \mathrm{len}(nums1)$, nums2.length $\le 105$
nums1.length == nums2.length
0 <= nums1i $\le \mathrm{len}(nums1)$
0 <= nums2i $\le \mathrm{len}(nums2)$
