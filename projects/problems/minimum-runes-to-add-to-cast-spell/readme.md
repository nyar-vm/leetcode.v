...
- **LeetCode**：[#3383 Minimum Runes To Add To Cast Spell](https://leetcode.com/problems/minimum-runes-to-add-to-cast-spell/)
- **难度**：Hard
- **标签**：Depth-First Search · Breadth-First Search · Union Find · Graph · Topological Sort · Array
...
2 <= n $\le 105$
1 $\le \mathrm{len}(crystals)$ <= n
0 <= crystals[i] <= n - 1
1 $\le \mathrm{len}(flowFrom)$ == flowTo.length <= min(2 * 105, (n * (n - 1)) / 2)
