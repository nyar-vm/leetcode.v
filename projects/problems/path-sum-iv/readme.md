...
- **难度**：Medium
- **标签**：Tree · Depth-First Search · Array · Hash Table · Binary Tree
...
The hundreds digit represents the depth d of this node, where 1 <= d $\le 4$.
The tens digit represents the position p of this node within its level, where 1 <= p $\le 8$, corresponding to its position in a full binary tree.
The units digit represents the value v of this node, where 0 <= v $\le 9$.
