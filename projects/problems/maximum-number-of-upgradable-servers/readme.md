...
- **LeetCode**：[#3155 Maximum Number Of Upgradable Servers](https://leetcode.com/problems/maximum-number-of-upgradable-servers/)
- **难度**：Medium
- **标签**：Array · Math · Binary Search
...
1 $\le \mathrm{len}(count)$ == upgrade.length == sell.length == money.length $\le 105$
1 <= count[i], upgrade[i], sell[i], money[i] $\le 105$
