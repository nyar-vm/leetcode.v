...
- **LeetCode**：[#3018 Maximum Number Of Removal Queries That Can Be Processed I](https://leetcode.com/problems/maximum-number-of-removal-queries-that-can-be-processed-i/)
- **难度**：Hard
...
1- We choose and remove nums[0] since 1 $\le 1$, then nums becomes [2,3,4,5].
2- We choose and remove nums[0] since 2 $\le 2$, then nums becomes [3,4,5].
3- We choose and remove nums[0] since 3 $\le 3$, then nums becomes [4,5].
4- We choose and remove nums[0] since 4 $\le 4$, then nums becomes [5].
...
**Output**：3
**Explanation**：We don't do any operation and process the queries as follows:
1- We choose and remove nums[0] since 2 $\le 2$, then nums becomes [3,2].
2- We choose and remove nums[1] since 2 $\le 2$, then nums becomes [3].
3- We choose and remove nums[0] since 3 $\le 3$, then nums becomes [].
...
1- We choose and remove nums[0] since 4 $\le 4$, then nums becomes [3].
2- We choose and remove nums[0] since 3 $\le 3$, then nums becomes [].
...
1 $\le \mathrm{len}(queries)$ $\le 1000$
1 <= nums[i], queries[i] $\le 109$
