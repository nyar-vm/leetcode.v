...
- **难度**：Medium
- **标签**：Tree · Depth-First Search · Breadth-First Search · Array · Hash Table · String
...
2 $\le \mathrm{len}(regions)$ $\le 104$
2 <= regions[i].length $\le 20$
1 <= regions[i][j].length, region1.length, region2.length $\le 20$
region1 != region2
